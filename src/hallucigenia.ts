...
const MENU: {
	...
	'watch mode': async () => {
		...
		info_anm.info(
			`detected changes on ${branches_to_clone.length} branches\n\n`
		);
		...
		for (const to_clone of branches_to_clone) {
			...
			if (success) {
				...
				const branch_script = script_list.find(
					(s) => s === to_clone.branch_name
				);
	...
	'check repositories connection': async () => {
		console.log(
	...
	'sync repository branches': async () => {
		const target_repo = REPOSITORIES.GetRepo(APP_STATE.GetTargetRepositoryId());
	...
	'repository side effects': async () => {
		...
		if (answer.startsWith('branch_') || answer === 'default script') {
			...
			const saved_script_content =
				target_repo.GetRepositoryScriptContent(b_name);
